...
io.on('connection', function (socket) {
    ...
    socket.on("message", function (msg) {
        ...
        if (msg.to != null) {
            if (clients[msg.to] !== undefined) {
                ...
                clients[msg.to].socket.emit("message", {from: currentID, to: msg.to, text: msg.text, date: Date.now()});
                if (currentID !== msg.to) {
    ...
    socket.on("chifoumi", function ({to, element}) {
        ...
        if (duel.result === null) {
            ...
            socket.emit("chifoumi", {
                text: duel.result.message + (exAequo ? '' : currentPlayerIsWinner ? " c'est gagné ! :grinning:" : " c'est perdu ! :frowning_face:"),
                date: Date.now()
            })
            clients[to].socket.emit("chifoumi", {
                text: duel.result.message + (exAequo ? '' : !currentPlayerIsWinner ? " c'est gagné ! :grinning:" : " c'est perdu ! :frowning_face:"),
