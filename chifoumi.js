...
class Duel {
    ...
    play(player, value) {
        if (player === null || value === null) { // null parameters
            return -1;
        }

        if (this.getPlayer(player) === null) { // player not exists
            return -2;
        }

        if (this.getPlayer(player).element !== null) { // player has aldready played
            return -3;
        }

        if (!Object.keys(elements).includes(value)) { // Incorrect element
            return -4;
        }
        ...
        for (let i of [0, 1]) {
            if (elements[players[i].element][players[(i+1)%2].element]) {
                return {
                    ...
                    message: `:${players[i].element}: ${elements[players[i].element][players[(i+1)%2].element]} :${players[(i+1)%2].element}:`
                }
            }
        }
