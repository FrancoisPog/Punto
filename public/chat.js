...
document.addEventListener("DOMContentLoaded", function (_e) {
    ...
    let autoCompleteChifoumi = {
        ...
        next : () => {
            ...
            autoCompleteChifoumi.index = (autoCompleteChifoumi.index + 1) % autoCompleteChifoumi.elements.length;
            return res;
        }
    ...
    btnSend.onclick = () => {
        ...
        if(text.startsWith("/")){
            if (/^\/chifoumi /.test(text)) {
                ...
                if (match === null) {
                    addMessage("Nombre de paramètre incorrecte", "[chifoumi]", "chifoumi", Date.now())
                    return;
                }

                let opponentMatch = match[1].match(/^@([^\s]+$)/);
                if (opponentMatch === null) {
                    addMessage("Adversaire incorrecte ( @adversaire )", "[chifoumi]", "chifoumi", Date.now());
                    return;
                }

                let opponent = opponentMatch[1];
                if (opponent === pseudo) {
                    addMessage("Vous ne pouvez pas vous défier vous même", "[chifoumi]", "chifoumi", Date.now());
                    return;
                }
                ...
                addMessage(`Invalid command - "${text.match(/\/\S*/)}"`,"[admin]",'system',Date.now());
                return;
            }
        }
        let to = text.match(/^@\S+/)
    ...
    function addMessage(text, from, type, date) {
        ...
        let msg = elt('p', {class: type}, `${hours}:${minutes}:${secondes} - ${from} : ${text}`);
